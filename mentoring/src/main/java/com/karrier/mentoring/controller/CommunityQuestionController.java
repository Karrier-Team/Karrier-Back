...
public class CommunityQuestionController {
    ...
    //질문 답변 등록 요청
    @PostMapping("/question/answer/new")
    ...
    public ResponseEntity<Object> deleteComment(@RequestParam("programNo") long programNo, @RequestParam("questionNo") long questionNo) {

        Question question = communityQuestionService.findQuestion(programNo, questionNo);
        if (question == null || question.getAnswerDate() == null) { //해당 프로그램 번호와 질문 번호에 해당하는 답변이 없을 때
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("no review error");
        }
        //사용자 email 얻기
        Object principal = SecurityContextHolder.getContext().getAuthentication().getPrincipal();
